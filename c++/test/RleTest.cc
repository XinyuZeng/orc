...
const size_t DEFAULT_MEM_STREAM_SIZE = 2 * 1024 * 1024; // 20MB
...
int main(int argc, char *argv[])
{
  ...
  MemoryOutputStream *memStreams[19];
  ...
  for (size_t i = 0; i < num_buffers; i++)
  {
    auto encoder = createRleEncoder(std::unique_ptr<BufferedOutputStream>(
                                        new BufferedOutputStream(*pool, memStreams[i], 500 * 1024, 1024)),
                                    isSigned,
                                    RleVersion_2,
                                    *pool,
                                    alignedBitPacking);
    ...
    //   encoder->flush();
    // }
    // encoder->add(&data[(data.size() / STRIPE_SIZE) * STRIPE_SIZE], data.size() % STRIPE_SIZE, nullptr);
    // encoder->flush();
  }
  ...
  for (size_t i = 0; i < num_buffers; ++i)
  {
    auto decoder = createRleDecoder(std::unique_ptr<SeekableArrayInputStream>(new SeekableArrayInputStream(
                                        memStreams[i]->getData(),
                                        memStreams[i]->getLength())),
    ...
    decode_time += (static_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - begin)).count();
  ...
  std::cout << "decode time (s): " << decode_time << std::endl;
