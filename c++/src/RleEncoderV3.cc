 ...
 */
uint32_t RleEncoderV3::percentileBits(int64_t* data, size_t offset, size_t length, double p, bool reuseHist) {
    if ((p > 1.0) || (p <= 0.0)) {
...
RleEncoderV3::RleEncoderV3(std::unique_ptr<BufferedOutputStream> outStream,
...
void RleEncoderV3::write(int64_t val) {
    if(numLiterals == 0) {
...
void RleEncoderV3::computeZigZagLiterals(EncodingOption &option) {
    assert (isSigned);
...
void RleEncoderV3::preparePatchedBlob(EncodingOption& option) {
 ...
 */
int64_t* RleEncoderV3::prepareForDirectOrPatchedBase(EncodingOption& option) {
    if (isSigned) {
...
void RleEncoderV3::determineEncoding(EncodingOption& option) {
...
void RleEncoderV3::writeValues(EncodingOption& option) {
    if (numLiterals != 0) {
...
void RleEncoderV3::writeShortRepeatValues(EncodingOption&) {
    int64_t repeatVal;
    if (isSigned) {
...
void RleEncoderV3::writeDirectValues(EncodingOption& option) {
...
void RleEncoderV3::writePatchedBasedValues(EncodingOption& option) {
...
void RleEncoderV3::writeDeltaValues(EncodingOption& option) {
    uint32_t len = 0;
...
void RleEncoderV3::writeInts(int64_t* input, uint32_t offset, size_t len, uint32_t bitSize) {
...
void RleEncoderV3::initializeLiterals(int64_t val) {
    literals[numLiterals++] = val;
    fixedRunLength = 1;
    variableRunLength = 1;
}
